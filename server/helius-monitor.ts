...
export class HeliusMonitor {
  ...
  private readonly RECENT_TOKEN_WINDOW_MS = 30 * 60 * 1000; // 30 dakika
  ...
  private async handleDexLP(signature: string) {
    try {
      ...
      // Son 30 dakikada çıkan symbol mi?
      const lastSeen = this.recentTokenSymbols.get(symbol);
      if (lastSeen && Date.now() - lastSeen < this.RECENT_TOKEN_WINDOW_MS) {
        console.log(`⏭️ [LP] ${symbol} son 30 dakikada görüldü, arayüzde gösteriliyor (otomatik alım yapılmayacak)`);
      ...
      // Eski symbol'leri temizle (30 dakikadan eski)
