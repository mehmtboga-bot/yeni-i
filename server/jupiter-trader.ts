...
export class JupiterTrader {
  ...
  // --- Retry yardımcısı ---
  // Varsayılan: 3 retry, deneme arası 500ms bekleme — timeout yok, her deneme kendi süresini alır
  ...
  async buy(input: { mintAddress: string; name: string; symbol: string; solAmount?: number }): Promise<Position | null> {
    ...
    try {
      // 3 retry, deneme arası 500ms bekleme
  ...
  async sell(positionId: string): Promise<Position | null> {
    ...
    try {
      if (pos.dex === "pumpswap") {
        // PumpSwap satışı — 3 retry, deneme arası 500ms bekleme
        const result = await this.withRetry(async () => {
        ...
        // Jupiter satışı — route yoksa PumpSwap'a fallback, her ikisi de 3 retry, deneme arası 500ms bekleme
        let jupiterOk = false;
        try {
          const result = await this.withRetry(async () => {
            const balance = await fetchBalance();
          ...
          // Jupiter route yok → PumpSwap fallback dene (3 retry, deneme arası 500ms bekleme)
  ...
  async sellHalf(positionId: string): Promise<Position | null> {
    ...
    try {
      ...
      if (pos.dex === "pumpswap") {
        // PumpSwap yarı satışı — 3 retry, deneme arası 500ms bekleme
        const result = await this.withRetry(async () => {
          const balance = await fetchBalance();
        ...
        // Jupiter yarı satışı — route yoksa PumpSwap'a fallback, deneme arası 500ms bekleme
        let jupiterOk = false;
        try {
          const result = await this.withRetry(async () => {
            const balance = await fetchBalance();
