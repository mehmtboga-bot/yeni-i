...
export class AutoTraderEngine {
  ...
  private checkAndSell() {
    ...
    for (const [recordId, record] of this.records.entries()) {
      ...
      // Likidite düşüş kontrolü kaldırıldı — sadece satış başarısızlığı rug olarak işaretlenir
  ...
  // ─────────────────────────────────────────────
  //  LİKİDİTE DÜŞÜŞ KONTROLÜ (Rug Pull) — DEVRE DIŞI
  // ─────────────────────────────────────────────

  private async checkLiquidityDrop(recordId: string, record: AutoTradeRecord): Promise<void> {
    // Likidite düşüş kontrolü devre dışı — sadece satış başarısızlığı rug olarak işaretlenir
    return;
  }
